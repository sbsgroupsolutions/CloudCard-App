...
static Reachability1 *_sharedReachability;
...
+ (Reachability1 *)sharedReachability
{
	if (!_sharedReachability) {
		_sharedReachability = [[Reachability1 alloc] init];
...
- (void)scheduleOnRunLoop:(NSRunLoop *)inRunLoop
{
	...
	if ([[Reachability1 sharedReachability] networkStatusNotificationsEnabled] == NO) {
		return;
	}
